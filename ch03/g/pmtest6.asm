...
.1:
	stosd				; [stosd] -> 以 eax 的内容填充 es:edi 指向的内存, 以4字节为单位. stosd 执行后 edi = edi + 4
...
.2:
	...
	mov	cr3, eax
	mov	eax, cr0		; ┓
	or	eax, 80000000h		; ┣  置cr0的PG位, 打开分页机制
	mov	cr0, eax		; ┛
	jmp	short .3
.3:
	nop
...
LABEL_SEG_CODE16:
	...
	mov	eax, cr0		; ┓
	and	eax, 7FFFFFFEh		; ┣  PE=0禁用保护模式(由实模式跳入保护模式时OS将PE置位), PG=0关闭分页机制
	mov	cr0, eax		; ┛
